...
#Dynamically identify the interface name
interface=($(ls /sys/class/net))
for(( i=0; i<"${#interface[@]}"; i++ ))
do
{
  value="lo"
  if [ ${interface[i]} == $value ]; then
    {
     ${interface[i]} 2>/dev/null
     break
    }
  fi
...
network_address=$(ip route list dev ${interface[i]} scope link | cut -d ' ' -f 1 | awk -F '[/ ]+' 'NR==2{print $1}')
